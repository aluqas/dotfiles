...
5. `homeImports` により `profiles/home/*` と `hosts/*/home.nix` が user environment を組み立てる
...
Darwin 固有の実装はここへ集約し、`hosts/macbook/*` は薄く保ちます。
...
host 固有の Home Manager 設定は `hosts/*/home.nix` に直接書きます。

- `home.stateVersion` と `programs.home-manager.enable` は `lib/hosts.nix` で共通注入
- 共有テーマ設定は `profiles/home/stylix/`
- host 固有の有効化（alacritty, vscode, spicetify など）は `hosts/macbook/home.nix` に直書き
...
- `hosts/oci-nixcloud/default.nix`
...
- theme asset: `profiles/home/stylix/rose-pine-moon.yaml`

原則:

- 配色や共通テーマ方針は `profiles/home/stylix/default.nix`
- アプリ固有の調整は `modules/home/*`
