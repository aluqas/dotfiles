  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.enable {
      ...
      environment.systemPackages = with pkgs;
        ...
        ++ lib.optionals cfg.rootless [
          rootlesskit
          slirp4netns
          dbus-broker
        ];

      # rootless nerdctl 用の environment variable
      # global env var ではなく alias にして、sudo / root の nerdctl を壊さないようにする
      environment.shellAliases = lib.mkIf cfg.rootless {
        nerdctl-rootless = "CONTAINERD_ADDRESS=/run/user/$UID/containerd/containerd.sock BUILDKIT_HOST=unix:///run/user/$UID/buildkit/buildkitd.sock nerdctl";
      };
      ...
      # nerdctl build 用に BuildKit を有効化する（rootless でなければ system-wide、rootless なら user unit）
      systemd.services.buildkitd = lib.mkIf (!cfg.rootless) {
      ...
      # Rootless BuildKit（user service）は手動管理か user-systemd 依存になるが、
      # たいていは rootless mode で `nerdctl` が見つからない場合に `buildkitd` を自動起動する。
      # ここでは tool だけを提供する。
      ...
      # この module が有効なら runtimes も既定で有効にする
      saqula.system.services.container.runtimes.enable = mkDefault true;
    })
  ];
}
