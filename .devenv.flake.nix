{
  inputs = let
    vars = {
      ...
      devenv_root = "/Users/saqula/dotfiles";
      project_input_ref = "path:/Users/saqula/dotfiles";
      devenv_dotfile = "/Users/saqula/dotfiles/.devenv";
      devenv_dotfile_path = ./.devenv;
      devenv_tmpdir = "/var/folders/jx/dppt4w5n3jdcl4t2z63pss8r0000gn/T/";
      devenv_runtime = "/var/folders/jx/dppt4w5n3jdcl4t2z63pss8r0000gn/T/devenv-d0fccad";
      ...
      hostname = "macbook";
      username = "saqula";
      git_root = "/Users/saqula/dotfiles";
      secretspec = null;
    };
  in
    {
    ...
    // (
      if builtins.pathExists (vars.devenv_dotfile_path + "/flake.json")
      then builtins.fromJSON (builtins.readFile (vars.devenv_dotfile_path + "/flake.json"))
      else {}
    );
  ...
  outputs = {nixpkgs, ...} @ inputs: let
    ...
    devenv =
      if builtins.pathExists (vars.devenv_dotfile_path + "/devenv.json")
      then builtins.fromJSON (builtins.readFile (vars.devenv_dotfile_path + "/devenv.json"))
      else {};
    ...
    mkDevenvForSystem = targetSystem: let
      ...
      pkgs = import nixpkgs {
        ...
        config = {
          ...
          allowUnfreePredicate =
            if (permittedUnfreePackages != [])
            then (pkg: builtins.elem (nixpkgs.lib.getName pkg) permittedUnfreePackages)
            else (_: false);
        };
        inherit overlays;
      };
      ...
      baseProject = pkgs.lib.evalModules {
        ...
        modules =
          ...
          ++ [
            ...
            (
              if builtins.pathExists (vars.devenv_dotfile_path + "/cli-options.nix")
              then import (vars.devenv_dotfile_path + "/cli-options.nix")
              else {}
            )
          ];
      };
      ...
      project = let
        ...
        resolveProfileExtends = profileName: visited:
          if builtins.elem profileName visited
          then throw "Circular dependency detected in profile extends: ${lib.concatStringsSep " -> " visited} -> ${profileName}"
          else let
        ...
        allPrioritizedModules =
          lib.imap0 (
            index: profileName: let
              ...
              typeNeedsOverride = type:
                ...
                in
                  ...
                  then
                    ...
                    in
                      if innerType != null
                      then typeNeedsOverride innerType
                      else false
                  else
              ...
              pathNeedsOverride = optionPath: let
                # Try direct option first
                directOption = lib.attrByPath optionPath null baseProject.options;
              in
                if directOption != null && lib.isOption directOption
                then typeNeedsOverride directOption.type
                else if optionPath != []
                then
                  ...
                  in
                    if parentOption != null && lib.isOption parentOption
                    then let
                      ...
                      elementType =
                        if freeformType ? elemType
                        then freeformType.elemType
                        else if freeformType ? nestedTypes && freeformType.nestedTypes ? elemType
                        then freeformType.nestedTypes.elemType
                        else freeformType;
                    in
              ...
              applyOverrideRecursive = config: optionPath:
                ...
                then config # Don't touch values with existing type metadata
                else if lib.isAttrs config
                then lib.mapAttrs (name: value: applyOverrideRecursive value (optionPath ++ [name])) config
                else if pathNeedsOverride optionPath
                then lib.mkOverride profilePriority config
                else config;
      ...
      build = options: config:
        lib.concatMapAttrs (
          name: option:
            ...
            else if builtins.isAttrs option && !lib.isDerivation option
            then let
